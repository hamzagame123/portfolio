...
function initializeApp() {
    // Keep the brand moment, but get reviewers into the prototype quickly.
    setTimeout(() => {
...
// Add to Cart
function addToCart() {
    const firstCartItem = document.querySelector('.cart-item');
    const firstQty = firstCartItem?.querySelector('.cart-item-controls span');
    if (firstQty) {
        firstQty.textContent = Math.min(99, (parseInt(firstQty.textContent, 10) || 0) + quantity);
        updateCartSummary();
    }
...
function updateCartBadge() {
    const totalItems = [...document.querySelectorAll('.cart-item-controls span')]
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (searchInput) {
        searchInput.addEventListener('input', (e) => {
            activeCategory = '';
            applySearchFilters(e.target.value);
        });
    }
});
...
function applySearchFilters(value) {
    ...
    const categoryTerms = {
        vegetables: ['tomato', 'lettuce', 'carrot', 'pepper', 'vegetable'],
        fruits: ['fruit', 'apple', 'berry', 'strawberr'],
    ...
    results.forEach(result => {
        ...
        const categoryMatch = !terms || terms.some(term => text.includes(term));
        result.style.display = categoryMatch && (!query || text.includes(query)) ? 'flex' : 'none';
    });
}
...
function updateCartSummary() {
    ...
    items.forEach(item => {
        ...
        const qty = parseInt(item.querySelector('.cart-item-controls span')?.textContent || 1);
        subtotal += price * qty;
    });
    ...
    const delivery = document.querySelector('.delivery-option.active .option-price')?.textContent.trim();
    const deliveryAmount = delivery === 'Free' ? 0 : 5.99;
    const total = subtotal + 0.99 + deliveryAmount;
    const totalEl = document.querySelector('.summary-row.total span:last-child');
    const checkoutBtn = document.querySelector('.checkout-btn');
    if (totalEl) totalEl.textContent = `$${total.toFixed(2)}`;
    if (checkoutBtn) checkoutBtn.textContent = `Proceed to Checkout - $${total.toFixed(2)}`;
}
...
document.addEventListener('click', (e) => {
    if (e.target.classList.contains('add-btn') || e.target.classList.contains('add-btn-small')) {
        ...
        const qty = cartItem?.querySelector('.cart-item-controls span');
        if (qty) qty.textContent = Math.min(99, (parseInt(qty.textContent, 10) || 0) + 1);
        updateCartSummary();
        updateCartBadge();
    }
});
...
function toggleGoing(btn) {
    ...
    if (!btn.dataset.baseCount && countEl) {
        const match = countEl.textContent.match(/\d+/);
        btn.dataset.baseCount = match ? match[0] : '0';
    }
    ...
    if (btn.classList.contains('active')) {
        ...
        if (countEl) {
            countEl.innerHTML = `<i class="ph-fill ph-users"></i> ${Number(btn.dataset.baseCount) + 1} going`;
        }
    } else {
        const goingText = btn.querySelector('.going-text');
        if (goingText) {
            goingText.textContent = btn.classList.contains('small') ? 'Going?' : 'I\'m Going';
        }
        if (countEl) {
            countEl.innerHTML = `<i class="ph-fill ph-users"></i> ${btn.dataset.baseCount} going`;
        }
...
function toggleGoingDetail(btn) {
    ...
    if (btn.classList.contains('active')) {
        ...
        showToast("You're going! Invite friends when you're ready.");
    } else {
        btn.innerHTML = "I'm Going <i class='ph ph-calendar-check'></i>";
        btn.style.background = '';
        showToast('Attendance removed');
    }
}
...
function openMaps() {
    showToast('Directions previewed in this prototype');
}

// Complete the visible market-detail actions with clear prototype feedback.
document.addEventListener('click', (event) => {
    const reminder = event.target.closest('.btn-reminder');
    if (reminder) {
        reminder.classList.toggle('active');
        const active = reminder.classList.contains('active');
        reminder.innerHTML = active
            ? '<i class="ph-fill ph-bell-ringing"></i> Reminder Set'
            : '<i class="ph ph-bell"></i> Remind Me';
        showToast(active ? 'Reminder set for Friday evening' : 'Reminder removed');
    }

    const invite = event.target.closest('.btn-invite');
    if (invite) {
        showToast('Invite flow opened');
        showScreen('messages');
    }
});
